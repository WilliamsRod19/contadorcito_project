...
<body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container mt-4">
                    <h1 class="mb-4">Editar Comprobante</h1>

                    <!-- Formulario de Edición de Comprobante -->
                    <form method="POST" action="../../controllers/purchases/editPurchaseController.php" enctype="multipart/form-data">
                        <!-- ID oculto -->
                        ...
                        <div class="mb-3">
                            ...
                            <select name="tipo_comprobante" id="tipo_comprobante" class="form-control" required>
                                ...
                                include '../../controllers/purchases/getType.php';
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="proveedor" id="proveedor" class="form-control" value="<?= ($comprobante['proveedor']) ?>" required>
                        </div>
    ...
    <script>
        document.querySelector('form').addEventListener('submit', function(event) {
            var eliminarPdf = document.querySelector('input[name="delete_pdf"]:checked');
            var eliminarJson = document.querySelector('input[name="delete_json"]:checked');
            var archivoPdf = document.getElementById('archivo_pdf');
            var archivoJson = document.getElementById('archivo_json');

            var validarArchivo = function(eliminarCheckbox, archivoInput, archivoExistente) {
                if (eliminarCheckbox && archivoExistente && archivoInput.files.length === 0) {
                    event.preventDefault();
                    alert("Si deseas eliminar un archivo, debes subir otro archivo.");
                    return false;
                }
                return true;
            };

            var archivoPdfExistente = "<?= !empty($comprobante['archivo_pdf']) ?>" === "1";
            var archivoJsonExistente = "<?= !empty($comprobante['archivo_json']) ?>" === "1";

            if (!validarArchivo(eliminarPdf, archivoPdf, archivoPdfExistente) ||
                !validarArchivo(eliminarJson, archivoJson, archivoJsonExistente)) {
                return;
            }
        });
    </script>
