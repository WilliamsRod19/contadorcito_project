...
function updateComprobante($id, $empresa_id, $tipo_comprobante, $numero_comprobante, $fecha_comprobante, $monto, $proveedor, $archivo_pdf, $archivo_json) {
    global $connection;
    $sql = "UPDATE tbl_Comprobantes_Compra SET 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($empresa_id) || empty($tipo_comprobante) || empty($numero_comprobante) || 
        empty($fecha_comprobante) || empty($monto) || empty($proveedor)) {
        die("Todos los campos son obligatorios.");
    }

    // Obtener los detalles del comprobante actual
    $sql = "SELECT archivo_pdf, archivo_json FROM tbl_Comprobantes_Compra WHERE id = :id";
    $stmt = $connection->prepare($sql);
    $stmt->bindParam(':id', $id);
    $stmt->execute();
    ...
    // Actualizar el comprobante en la base de datos usando la función de actualización
    if (updateComprobante($id, $empresa_id, $tipo_comprobante, $numero_comprobante, $fecha_comprobante, $monto, $proveedor, $pdf_path, $json_path)) {
        header('Location: ../../views/purchases/indexPurchases.php');
        exit;
    } else {
        die("Error al actualizar el comprobante.");
    }
}
